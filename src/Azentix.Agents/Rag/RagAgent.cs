...
{
    Task<string> SearchAsync(string query, string collection = "default",
...
public class RagAgent : IRagAgent
{
    ...
    public RagAgent(IVectorMemory memory, EmbeddingClient embeddings,
        ILogger<RagAgent> logger)
    { _memory = memory; _embeddings = embeddings; _logger = logger; }

    public async Task<string> SearchAsync(string query,
        string collection = "default", int topK = 5, CancellationToken ct = default)
    {
        _logger.LogInformation("RAG search: '{Q}' in '{Col}'",
            query[..Math.Min(60, query.Length)], collection);
